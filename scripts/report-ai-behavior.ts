...
function report(metrics: AiBehaviorMetrics): string {
  return [
    ...
    `stack-offs (all command forms) ${metrics.stackOffActions}`,
    `flop SPR>20 facing <=0.5 pot ${percent(metrics.highSprFlopFacingSmall.rate)} (n=${metrics.highSprFlopFacingSmall.decisions}, stack-offs=${metrics.highSprFlopFacingSmall.stackOffs})`,
    `ordinary wager denomination violations ${metrics.ordinaryWagerAmounts.denominationViolations}/${metrics.ordinaryWagerAmounts.count} (${percent(metrics.ordinaryWagerAmounts.denominationViolationRate)})`,
    `ordinary wager final digits      ${JSON.stringify(metrics.ordinaryWagerAmounts.finalDigitCounts)}`,
    `ordinary wager quantization abs rack/target ${metrics.ordinaryWagerAmounts.meanAbsRackAdjustment.toFixed(2)} / ${metrics.ordinaryWagerAmounts.meanAbsTargetAdjustment.toFixed(2)} chips (max ${metrics.ordinaryWagerAmounts.maximumAbsRackAdjustment.toFixed(2)} / ${metrics.ordinaryWagerAmounts.maximumAbsTargetAdjustment.toFixed(2)}, n=${metrics.ordinaryWagerAmounts.quantizationSamples})`,
    ...
    `hero session completion     ${metrics.completedEvents} / ${metrics.seeds} (capped ${metrics.cappedEvents})`,
    `full field completion       ${metrics.fieldCompletedEvents} / ${metrics.seeds}`,
    ...
    `field hands to heads-up     ${metrics.fieldHandsToHeadsUp.mean.toFixed(1)} / ${metrics.fieldHandsToHeadsUp.median} (n=${metrics.fieldHandsToHeadsUp.samples})`,
    `hero hands to heads-up      ${metrics.handsToHeadsUp.mean.toFixed(1)} / ${metrics.handsToHeadsUp.median} (n=${metrics.handsToHeadsUp.samples})`,
    `hands to hero finish        ${metrics.handsToFinish.mean.toFixed(1)} / ${metrics.handsToFinish.median} (n=${metrics.handsToFinish.samples})`,
  ].join("\n");
}
