...
exports.defaultProgress = exports.defaultSettings = exports.PERSISTED_ACTION_IDS = exports.ACTION_MAP_VERSION = void 0;
exports.normalizeControlBindingOverrides = normalizeControlBindingOverrides;
exports.normalizePersistedSettings = normalizePersistedSettings;
exports.normalizePersistedProgress = normalizePersistedProgress;
exports.validateCurrentPersistedSettings = validateCurrentPersistedSettings;
exports.validateCurrentPersistedProgress = validateCurrentPersistedProgress;
/**
 ...
 */
exports.ACTION_MAP_VERSION = 1;
exports.PERSISTED_ACTION_IDS = [
    "menu.up",
    "menu.down",
    "menu.left",
    "menu.right",
 ...
 */
function normalizeControlBindingOverrides(value) {
    if (!isRecord(value))
        return undefined;
    ...
    return { version: exports.ACTION_MAP_VERSION, keyboard, gamepad };
}
function normalizePersistedSettings(value) {
    const source = isRecord(value) ? value : {};
    const controlBindings = normalizeControlBindingOverrides(source.controlBindings);
    return {
        masterVolume: boundedNumber(source.masterVolume, 0, 100, exports.defaultSettings.masterVolume),
        muted: booleanOr(source.muted, exports.defaultSettings.muted),
        musicVolume: boundedNumber(source.musicVolume, 0, 100, exports.defaultSettings.musicVolume),
        effectsVolume: boundedNumber(source.effectsVolume, 0, 100, exports.defaultSettings.effectsVolume),
        fullscreen: booleanOr(source.fullscreen, exports.defaultSettings.fullscreen),
        reducedMotion: booleanOr(source.reducedMotion, exports.defaultSettings.reducedMotion),
        reducedMotionExplicit: booleanOr(source.reducedMotionExplicit, exports.defaultSettings.reducedMotionExplicit),
        dealSpeed: dealSpeeds.has(source.dealSpeed)
            ? source.dealSpeed
            : exports.defaultSettings.dealSpeed,
        colorAssist: booleanOr(source.colorAssist, exports.defaultSettings.colorAssist),
        cameraSensitivity: cameraSensitivities.has(source.cameraSensitivity)
            ? source.cameraSensitivity
            : exports.defaultSettings.cameraSensitivity,
        cameraView: cameraViews.has(source.cameraView)
            ? source.cameraView
            : exports.defaultSettings.cameraView,
        autoCameraMovement: booleanOr(source.autoCameraMovement, exports.defaultSettings.autoCameraMovement),
        menuMotion: motionIntensityOr(source.menuMotion, exports.defaultSettings.menuMotion),
        roomMotion: motionIntensityOr(source.roomMotion, exports.defaultSettings.roomMotion),
        cameraMotion: motionIntensityOr(source.cameraMotion, exports.defaultSettings.cameraMotion),
        tableMotion: motionIntensityOr(source.tableMotion, exports.defaultSettings.tableMotion),
        transitionMotion: motionIntensityOr(source.transitionMotion, exports.defaultSettings.transitionMotion),
        interfaceScale: interfaceScales.has(source.interfaceScale)
            ? source.interfaceScale
            : exports.defaultSettings.interfaceScale,
        spatialScene: booleanOr(source.spatialScene, exports.defaultSettings.spatialScene ?? false),
...
function normalizePersistedProgress(value) {
    ...
    const currentStreak = nonNegativeInteger(source.currentStreak, exports.defaultProgress.currentStreak);
    const bestStreak = Math.max(currentStreak, nonNegativeInteger(source.bestStreak, exports.defaultProgress.bestStreak));
    return {
        onboardingCompleted: booleanOr(source.onboardingCompleted, exports.defaultProgress.onboardingCompleted),
        playChipsAcknowledged: booleanOr(source.playChipsAcknowledged, exports.defaultProgress.playChipsAcknowledged),
        playerName: typeof source.playerName === "string" &&
            source.playerName.trim().length > 0
            ? source.playerName.slice(0, 48)
            : exports.defaultProgress.playerName,
        decisionElo: finiteNumber(source.decisionElo, exports.defaultProgress.decisionElo),
        mathElo: finiteNumber(source.mathElo, exports.defaultProgress.mathElo),
        tournamentElo: finiteNumber(source.tournamentElo, exports.defaultProgress.tournamentElo),
        trainingCompleted: nonNegativeInteger(source.trainingCompleted, exports.defaultProgress.trainingCompleted),
        currentStreak,
        bestStreak,
        totalDecisionMs: Math.max(0, finiteNumber(source.totalDecisionMs, exports.defaultProgress.totalDecisionMs)),
        results,
        unlockedCircuit: Math.max(1, nonNegativeInteger(source.unlockedCircuit, exports.defaultProgress.unlockedCircuit)),
 ...
 */
function validateCurrentPersistedSettings(value) {
    if (!isRecord(value))
        return invalid("The settings value is invalid.");
    for (const key of [
        "masterVolume",
        "musicVolume",
        "effectsVolume",
    ...
    if (!dealSpeeds.has(value.dealSpeed)) {
        return invalid("The dealSpeed setting is invalid.");
    }
    if (!optionalMember(value.cameraSensitivity, cameraSensitivities) ||
...
function validateCurrentPersistedProgress(value) {
    if (!isRecord(value))
        return invalid("The progress value is invalid.");
    ...
    if (!Number.isFinite(value.totalDecisionMs) ||
...
function normalizeDeviceOverrides(value) {
    const result = {};
    if (!isRecord(value))
        return result;
    for (const id of exports.PERSISTED_ACTION_IDS) {
...
function validateControlBindingOverrides(value) {
    if (value === undefined)
        return true;
    if (!isRecord(value))
        return false;
    if (value.version !== undefined && value.version !== exports.ACTION_MAP_VERSION) {
        return false;
    }
...
function isValidBindingToken(value) {
    return (typeof value === "string" &&
        value.length > 0 &&
        value.length <= MAX_BINDING_TOKEN_LENGTH &&
        !/[\u0000-\u001f\u007f]/u.test(value));
}
function normalizeReviewTotals(value) {
    const source = isRecord(value) ? value : {};
    const decisions = nonNegativeInteger(source.decisions, 0);
    return {
        roundsReviewed: nonNegativeInteger(source.roundsReviewed, 0),
        decisions,
        bestDecisions: Math.min(decisions, nonNegativeInteger(source.bestDecisions, 0)),
        totalRegretBigBlinds: Math.max(0, finiteNumber(source.totalRegretBigBlinds, 0)),
    };
}
function normalizeCareerResult(value) {
    if (!isRecord(value))
        return undefined;
    ...
    return {
        ...
        qualifyingPlaces: Math.max(0, nonNegativeInteger(value.qualifyingPlaces, 0)),
...
function normalizeCareerTrack(value) {
    ...
    if (Array.isArray(source.results)) {
        for (const entry of source.results) {
            ...
            if (!result)
                continue;
            const existing = results.findIndex((candidate) => candidate.eventId === result.eventId);
            if (existing >= 0)
    ...
    const activeEventId = typeof source.activeEventId === "string" && source.activeEventId.length > 0
...
function normalizeTrainingResult(value) {
    if (!isValidTrainingResult(value))
        return undefined;
    return {
...
function isValidTrainingResult(value) {
...
function isValidCurrentCareer(value) {
    if (value === undefined)
        return true;
    if (!isRecord(value))
        return false;
    return ["normal", "rational"].every((track) => {
        ...
        if (!isRecord(candidate))
            return false;
        if (candidate.activeEventId !== undefined &&
...
function isValidCurrentCareerResult(value) {
    if (!isRecord(value) ||
...
function motionIntensityOr(value, fallback) {
    return motionIntensities.has(value)
        ? value
        : fallback;
}
function optionalMember(value, allowed) {
