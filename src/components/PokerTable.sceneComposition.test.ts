...
describe("scene-ready table composition", () => {
  it("reveals the ready WebGL furniture without unmounting the DOM surface", () => {
    const readyContract = styles.match(/\.poker-table\[data-spatial-scene="ready"\] \{([\s\S]*?)\n\}/)?.[1] ?? "";
  ...
  it("fades only duplicate decorative furniture, preserving readable DOM facts", () => {
    const decorationFade = styles.match(/\.poker-table\[data-spatial-scene="ready"\]::before,([\s\S]*?)\n\}/)?.[1] ?? "";
    ...
    const feltRingReadyRule = styles.match(/\.poker-table\[data-spatial-scene="ready"\] \.felt-ring \{([\s\S]*?)\n\}/)?.[1] ?? "";
