...
- [ ] Generation guarantees enforced end-to-end, with the reviewer-metadata policy decided in writing. **Half done: the policy is decided and written, the end-to-end enforcement waits on generation.** `docs/training-scenario-review-policy.md` records what each field claims (`source.verificationStatus` = the mathematics has been checked; `review.status` = this specific scenario's wording and framing have been signed off), why they are distinct, and that no script, generator, or migration may promote either to its signed-off value — approval is a human act recorded by a human. **One enforcement hole was found and closed while writing it:** the two fields were validated in isolation, so a scenario could claim `verified` mathematics while its review was still `pending` — a claim of human sign-off with nobody attached, and precisely what a generator with no human in the loop produces by default. `validateReviewProvenanceAgreement` now rejects that. The rule runs one way only: an approved review may precede verified provenance, because approving the wording does not verify the arithmetic.
...
- [x] **The gate extends past the table to every other screen.** Added 2026-07-26 as `audit-packaged-screen-layout.mjs`, covering first-run setup, home menu, credits, settings, mode select, poker reference, career lobby, arrival fly-through, live table, and the pause overlay at 1280×720, 1600×900, and 1920×1080 — ten screens, thirty measurements — with a screenshot of each written to `work/screen-layout/` so the aesthetic half of a review can be done by looking. Not covered, because both need a completed event: the ceremony and the hand-review screen. Those are exercised functionally by `mode-completion` but their geometry is not measured.

  The central check is deliberately **not** rectangle intersection. Boxes may overlap and still be perfectly usable — nested, transparent where they meet, or `pointer-events: none` — so the audit asks the browser the same question the player's mouse asks: `elementFromPoint` at each control's centre and four corners. If something else answers, the control is unclickable and the report names what covered it. Alongside it: controls outside the viewport that no scrollable ancestor can reach, the WCAG 2.2 AA 24×24 target-size minimum with the standard's spacing exception, and text clipped by an overflow-hiding ancestor.

  **Three defects found and fixed, all invisible to the existing suite because jsdom has no layout engine:**
  - **"Credits & licenses" on the main menu could not be clicked at any viewport.** It and the play-chip boundary notice were both anchored bottom-right at nearly the same offsets, and the notice's opaque pill (`z-index: 12`) sat exactly on top of the link (`z-index: 3`), swallowing every click. Keyboard focus still reached it, which is why it went unnoticed — the link looked reachable and simply did nothing. It now sits above the notice sharing its pill styling; it was **not** moved to the opposite corner because that corner of the menu art is a brightly lit chip where a plain text link is unreadable.
  - **The venue marquee's logo covered the first letter of its own title**, rendering "Championship Series" as "hampionship Series" on every screen that shows the marquee. Measured in the packaged build: the logo spans 528→570px and the title starts at 558px — a 12px overlap. The marquee's horizontal padding now clears the logo, kept symmetric so the flex centring is undisturbed.
  - **`.tour-lobby__progress` was applied in the markup and never styled at all**, so the career lobby read "0 of 5 events qualifiedNext up: Local Qualifier" — two bare inline boxes with no whitespace between them. It is now a flex row with a gap and the interpunct separator already used elsewhere in the game.
  - **The poker reference scrolled sideways by 15px at every viewport.** `.night-shell` is `width: 100vw`, which includes the vertical scrollbar and is invisible only because the shell also sets `overflow: hidden`. `.reference-shell` opts back into scrolling with `overflow: auto`, grows a scrollbar, and is then exactly one scrollbar wider than the space it has. Capped with `max-width: 100%`, which excludes the scrollbar and is a no-op on every non-scrolling screen. Found only after the credits fix made the credits screen reachable, which is what prompted adding credits and the reference to the audit's route.

  **Reviewed by eye as well as by gate**, from the captured screenshots. Things that looked wrong and are not, recorded so they are not "fixed" later by someone reading the same screenshot: the **"Table view / Centered" camera readout is dim because it is `disabled` when the camera is already centred** — the component documents this as deliberate, so that it stays a live readout instead of disappearing; the **`.night-scene` backdrop is completely static**, with no `animation` declaration anywhere in its ~350 lines of CSS, which makes the menu tier's `.night-scene` rules dead weight and leaves mode select, credits, and the other shells built on it visually still while the home menu breathes (recorded in the presentation audit's `observations.staticMenuDecoration`, not failed — it is a design choice to revisit, not a defect); and the **first-run setup's primary actions sit below the fold at 1280×720**, which is reachable because the panel scrolls, but the only affordance is a thin scrollbar, so a first-time player at the minimum supported size may not realise there is more.
...
- [x] Every Part I acceptance criterion is verified in the **packaged** build, not only the dev renderer. **Closed 2026-07-26.** Nine CDP audits already covered: the renderer booting over the custom protocol (`render-smoke`), completing a full event in all three modes through ordinary controls (`mode-completion`), zero network egress across a representative play walk-through (`network`), safe mode and its continue path (`safe-mode`), corrupt-save recovery (`save-recovery-smoke`), graceful asset/font/audio-device fault handling (`asset-fault-smoke`), the lifecycle bridge's absence from the normal preload (`lifecycle-bridge-security`), input across mouse/keyboard/gamepad plus table geometry at five viewports and every interface scale (`input-smoke`), and flash luminance (`flash-capture`). The five surfaces that were asserted only in jsdom or against source are now packaged too:
  - **Motion tiers, direction, text expansion, and fly-through depth** — `audit-packaged-presentation-surfaces.mjs`. The reason these had to move out of jsdom is that **jsdom has no layout engine and no real cascade**: it cannot say that a duration resolved to `0s`, that a transform actually scaled one table smaller than another, or that a heading is clipped. Measured: all 15 settings-control-to-root-attribute pairs correct; Reduce motion forces all five surfaces off and gives the per-surface preferences back; and every tier resolves to genuinely different computed style — room `1.7s / 0.5s / 0s`, camera `0.32s / 0.12s / 0s`, table `timer-spin 3.2s / 7s / none`, transition `room-progress-arrival 1.65s / 0.9s / none`, menu on the home art. Fly-through: 3 tables at 3 distinct depths and 3 distinct resolved scales. Direction is a **layout-robustness** gate, not a translation gate — the build ships one locale, so it flips `dir` and expands every text node ~40% against the real DOM and measures clipping and overflow. Where a surface is transient it measures a probe element carrying the real class, and the report records `method: "live"` vs `"probe"` rather than letting the weaker evidence pass for the stronger.
  - **Hand review and career travel** — added to `mode-completion`'s Normal run, the one place in the suite where a real ceremony exists. Review must derive an actual timeline rather than sit on its placeholder or error branch, and must carry its approximation notice: measured 151 decisions, 4 segment groups, notice present. Travel ran local → regional across 5 stops and delivered the player onward; when a result unlocks no next event it is recorded as `skipped` **with the reason**, because a hero who busted early proves nothing about travel and the report should say which run it was.
- [ ] Verification covers normal and heavy CPU load, all supported resolutions, all UI scales, reduced motion, window blur, minimize, suspend/resume, and screen lock where possible. **Mostly closed 2026-07-26; only suspend/resume and screen lock remain.** Resolutions and interface scales are covered by the packaged input smoke and reduced motion by the presentation audit. `audit-packaged-lifecycle-load.mjs` now covers the rest at a live table. Minimize and restore are driven by **real Win32 `ShowWindow` calls on the app's own window handle**, not CDP emulation — emulation tells the renderer a story about its visibility and does not exercise Chromium's actual background throttling, which is the behaviour the power requirement depends on. Measured: 150 fps visible → **0 fps minimized**, `visibilityState: hidden`, `hasFocus: false`, and the hand's rendered signature unchanged across the hidden interval (play really does freeze while the player is away) → 170 fps on restore with the table still up. Under 11 of 12 cores saturated: worst CDP round trip 2 ms, 170 fps. **Still open:** suspend/resume and screen lock, which need machine states a headless run cannot enter without disrupting the machine it runs on; they stay listed under E26-001 and the audit's report names them in a `notCovered` field rather than quietly implying coverage.
  - Worth recording: this audit is the only one that launches with `windowsHide: false`. Every other packaged audit hides the window, and that is genuinely suppressive rather than cosmetic — measured on this build, a hidden launch leaves **every** process in the tree reporting `MainWindowHandle = 0`. The first run of this audit reported honestly that it could not find a window to minimize instead of substituting an emulated signal.
