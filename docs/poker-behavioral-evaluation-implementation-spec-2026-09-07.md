...
Date: 2026-09-07; contract revised 2026-09-08. Governing methodology: [approved design](poker-behavioral-evaluation-design-2026-09-05.md). Intended executor: Luna Max. This document specifies future implementation of **all T1–T15**, not a policy rebalance. The September 8 revision is documentation only and does not start those tasks. MUST/MUST NOT are acceptance requirements; proposed new module names below are deliberate allocations, not claims that those modules already exist.
...
Initial inspection was at `10952e3ae1ee7c87ab4d1632e36338bc20bd2955`; the September 8 revision starts from clean local `main` at `778f7b88af2fde875cc4a20e15b694a17de2f142`. Earlier Blackjack modifications and the September 5 design's dirty-tree statements are historical. Recheck and preserve any unrelated work at implementation time. This specification does not re-run the old audits or imply new statistical results.

One pure action-support assessment serves behavioral evaluation and the Game Review prototype. Mechanical validity, model support, action unusualness, EV quality and consumer authority remain distinct. The shared assessment consumes canonical facts and scoped evidence; it neither estimates universal poker plausibility nor chooses/vetoes actions. T2 owns the source-safe contract and deterministic assessment, T5 captures provenance, T6 reports it, T7 supplies independent model evidence, and T8 consumes the same assessment for confidence and grading eligibility. T10/T11 may evaluate its claims through blinded evidence, never grant live authority. The future Strategic Sanity intervention progression in section 6.4 is outside T1–T15; T15 verifies the seam and absence of live adoption, not an implemented guard.
...
* Current `buildRange` in `rational.ts` uses observed action counts/aggression; its range-weight calculation does not consume exact observed wager amounts. Thus existing action conditioning alone cannot certify size-conditioned range support. T7 records this limitation; this contract does not change that production formula.
...
| T6 Conditional statistical report | T3, T4, T5 | T8, T11, T12, T14, T15 | Measurement/tooling | Yes; generic aggregate/cluster statistics and action-support coverage reducer. |
...
| T8 Game Review evidence prototype | T2, T6, T7 | T15 | Pure prototype/source types/tests | Yes; source consumes pure support/provider ports; offline runner uses T6 diagnostic reducer. |
...
Create **new** pure shared `src/lib/pokerActionSemantics.ts` (T2), `src/lib/actionSupport.ts` (T2: `assessActionSupport`, scoped contracts and safe projection), `src/modes/reviewEvidence.ts` (T7–T8 types/math), `src/modes/handReviewPrototype.ts` (T8), and if needed `src/modes/handReviewReplay.ts` (T8 shared traversal). Source modules must be production-safe even when only offline callers use them. `actionSupport.ts` imports source semantics only; it accepts structured evidence through arguments, never loads reference files, hashes artifacts with Node, calls providers or imports scripts. T7 produces enriched evidence through this existing T2 contract; T8 never creates a second outlier classifier. T6 declares a generic action-audit row contract without importing T8; the T8 offline runner adapts prototype DTOs to it (direct T6→T8 dependency). No new T16 or guard module is allocated.
...
| T6 | `statistics.ts`, `metrics.ts`, `comparison.ts`, `report.ts` | `clusterBootstrap`, `aggregateBehavior`, `aggregateActionAudit`, `compareRuns`, `renderEvaluationReport` |
...
`BehavioralDecisionEvent` required fields: version/semanticsVersion; `decisionId`; `reproduction`; `policy`; `preState` (`BettingRoundState`, internal only); `actorView` (`PlayerInformationSet`); `strata`; `legal` (`LegalActionSet`); `candidates` (array below); `chosen` (`CanonicalAction`); `geometry`; `postcondition` (`pass|fail`, independent reconciliation errors, execution hash); `opportunityIds`; `traceRef`; scoped `actionSupport` for candidates/selected action and internal `supportTraceLink` (section 4.7). Candidate-only common-state evaluations set `execution: "counterfactual"` and `postcondition` checks a copied transition; real records use `execution: "observed"`. Never pretend a sampled candidate was executed in the population experiment.
...
Use a discriminated `ReviewerInputV2`, separate from `PublicHandHistory`. Common required fields: `schemaVersion`, opaque `caseId`, `task`, `rules` (holdem/no-limit, actual forced bets, rack/medium, objective), `publicTimeline` with street-appropriate boards, `selectedAction`, `legalActions` (canonical), raw and normalized `geometry`. No `Reproduction`, source paths, policy/model names, flags, truth hashes or seed. Initial blinded inputs also exclude shared support conclusions and private diagnostic evidence; the diagnosis-only exception below does not relax hidden-information redaction. Stable aliases `P1`…`P6` replace player IDs and names consistently within a bundle.
...
* `task: "diagnosis"`: actor-view decision plus selected verified ranges/decomposed values/support evidence. Requires locked initial verdict ID; store as a new phase, never overwrite initial verdict. Outcome remains hidden; full-truth settlement audit is a different tool. Shared support evidence is allowed only in this post-verdict phase and only after observer-safe projection.
...
The DTO also requires the four Review additions in section 4.7; their public projection is mandatory. Internal trace links are returned only by the offline runner, never embedded in a user-facing DTO.
...
### 4.7 Shared support assessment contract (T2 producer seam; T5–T8 consumers)

`ActionSupportInput` and `ActionSupportAssessment` live in `src/lib/actionSupport.ts`. T2 also owns their plain evidence shapes, including `ConditioningEvidence` and the source-safe shape of `ResponseSupport`; offline contracts import/re-export these, never the reverse. Export `assessActionSupport` and `projectActionSupportForObserver`; the latter rejects mismatched observer scopes rather than laundering a privileged assessment. Use explicit versioned enums, required nullable evidence with reasons, and deterministic outputs. Missing evidence never defaults to ordinary/high support. An assessment has no action replacement, grade, EV estimate or permission field other than literal `authority: "evidence_only"`. Compute input identity from the canonical input payload excluding its own identity field; validate it before offline hashing/joining.

| Contract / required fields | Producer → consumer and interpretation |
|---|---|
| Input `schemaVersion`, `assessmentVersion`, `semanticKey`, `observerScope`, `evidenceScopeId`, `modelId`, `modelVersion`, `objective`, `inputIdentity` | T2 constructs canonical facts; offline T1 hashes the full scoped payload. Source uses a stable canonical serialization identity. Scope identifies observer-visible information and model assumptions; IDs alone cannot certify support. T6/T7/T8 compare only identical scopes/versions/payloads. |
| Input `action`, `geometry`, `street`, `counts`, `position`, `publicPrefix`, `reopening`, `boundaryOrigin` | T2/T5 capture existing semantics before the action. Geometry includes current/pre-aggression pot, actual call, target, invested, raise-by, investment/pot, raise-over-pot-after-call, commitment fraction, pairwise matchable exposure, SPR/depth and stack-off class. Prefix includes prior aggression and ordered street actions. Boundary origin is ordinary proposal, exact call, minimum, all-in, forced amount or unknown with reason. T2 assessment and T7/T8 reuse these values, never infer them from a legacy amount alone. |
| Input `policyEvidence`, `responseEvidence`, `referenceEvidence` | T5 supplies available candidate probability/rank, sizing/quantization audit and selection provenance; T7 supplies branch/conditional counts, fallback status, conditioning records and per-model support/sensitivity results; T12 supplies actual Normal distribution/deviation evidence. Each unavailable item is explicit before its producer exists. Consumers cannot substitute Rational probabilities for Normal or policy popularity for independent support. |
| Output `mechanicalValidity`, `modelSupport`, `actionEvidence`, `reasonCodes`, `measurements`, `limitations`, `authority`, plus input identity/scope/version | T2 `assessActionSupport` produces mechanical `valid/invalid/unknown`; model `supported/sparse/assumption_sensitive/ood/unavailable`; action `ordinary/supported_unusual/anomalous/unknown`. These are orthogonal, not a severity ladder. Every conclusion carries evidence paths, measurements with units and basis; unknown needs missing-evidence reasons. T6 flags/reports and T8 confidence consume exactly this output. No universal percentage. |
| `ConditioningEvidence` per model: existing prefix hash, sizing signature/status plus `dimensions`, `domainEvidence`, `limitations` | T7 provider produces; T2 assessment/T8 consume. Dimensions are action type, raw target/investment, normalized sizing, ordered prior sequence, street, position, depth/matchable exposure and relevant tournament context. Each records observed value, actually used representation, status (`conditioned`, `assumed`, `unsupported`, `not_applicable`), basis/reference and justification. Domain evidence identifies supported range/response domain and any extrapolation. A hash or claimed status without these records is insufficient. |
| Internal `SupportTraceLink`: event/decision/candidate/semantic IDs, reproduction/trace refs, public replay digest/ordinal, assessment revision refs, investigation state and cause evidence | T5 captures join; T7 adds immutable evidence revisions; T6 retains it through every exclusion; T8 offline runner resolves replay association. T12 enriches Normal trace in a new artifact. Trace absent for old replay is explicitly `unavailable_historical_trace`, never fabricated. Incorrect replay joins fail verification. |
| `PublicActionSupportView`: opaque public action ID, public semantic facts, observer-scoped assessments, safe model limitations, conditioning summary and trace availability | T2 safe projection and T8 observer-scoped construction → prototype DTO. No internal event hashes, seeds, paths, opponent hole cards, private policy ranks or private-information-derived conclusions. Opaque public replay IDs map to internal links only in offline artifacts. A fresh assessment uses only information available to Hero at that historical node; never sanitize a bot's own-card assessment merely by deleting its cards. |
| Review additions `observedActionSupport`, `candidateActionSupport`, `conditioningEvidence`, `recommendationScope` | T8 produces from T2/T7, consumed by its DTO/display and offline runner. Observed entries cover opponent prefix actions relevant to range inference, not only Hero's candidate menu. Recommendation scope names supported models, robustness/limitations, and keeps numerical and strategic confidence separate. Public conditioning evidence contains only observer-safe model assumptions, not the actual hidden deal. |
| `ActionAuditRow`: canonical action kind, encounter ID/family/block/objective, grade eligibility/reason, support status, regret bounds, nullable grade, internal trace link | T6 owns generic schema/reducer; T8 offline runner supplies review rows. T6 reports counts/distributions under section 7.4. This row is not a T3 opportunity and does not change behavioral denominators. |

Reason-code vocabulary initially covers invalid transition/replay mismatch, missing evidence, unsupported conditioning/domain/objective, sparse call/re-raise branch, empty fallback, sensitivity disagreement, extreme geometry needing scrutiny, independently supported unusual reference, state-derived boundary, and ordinary evidence within a declared validated domain. Register code version, required measurements/evidence paths and public eligibility. A size-bin code is a retrieval/scrutiny signal only. Model support comes from declared reference/domain evidence, not numerical convergence, raw size, policy rank or critic opinion. `supported_unusual` requires positive scoped support; ordinary also needs a declared basis. No new calibrated rarity threshold is invented in T2.

Deterministic assessment steps (A02/A07): validate the fact schema and reconcile the canonical transition through the T2 engine adapter. A rejected raw command has a separate invalid-attempt frame with nullable canonical action/key and validation errors, not a fabricated successful `BehavioralDecisionEvent`. Evaluate support per named model; a sensitivity bundle carries a versioned bundle ID and explicit member-model assessments. Explicitly outside a declared domain yields `ood`; missing required domain/conditioning evidence yields `unavailable`; required sampled branches with missing observations/fallback yield `sparse`; demonstrated disagreement under a declared supported sensitivity bundle yields `assumption_sensitive`; otherwise positive reference/domain evidence yields `supported`. Preserve every reason when multiple limitations apply in that order, without turning mechanical validity into strategic support. Structurally absent response branches in a closed-betting exact reference are not sparse: mark them not applicable with proof of closure. Action evidence is supported unusual when scoped positive support coexists with an explained unusual-geometry/reference signal, ordinary only with a documented ordinary-domain basis, anomalous for an explained unresolved discrepancy requiring investigation, otherwise unknown. Existing section 6.3 retrieval measurements can trigger scrutiny but cannot set domain support or EV quality. Test each branch and combinations; future calibrated detectors must reuse this reason/evidence contract.

The same serialized input must produce identical assessments for both consumers (A02/A15). Different observers or model domains may yield different conclusions only with those scopes explicit; consumers cannot relabel identical `ood` evidence as high strategic confidence. Reports can show conditional values under an unsupported model, clearly conditional and excluded from robust grading. Supported 5x/10x overbets remain legal and potentially strong; missing calibration increases scrutiny without automatic invalidity, loss, suppression or exclusion (A04/A07/A08).

Internal investigations classify candidate generation, wager generation, quantization, canonicalization/stack semantics, response support, utility algebra, penalties/bonuses, stochastic selection, Normal deviation, range conditioning, tournament adjustment, legal-action construction, replay/state corruption or other with explanation. Store suspected/confirmed/rejected/unknown cause with evidence and affected stage; multiple causes are allowed. Preserve raw proposal → sizing → quantization → canonical candidate → response/components → probability/selection → executed transition. Do not consume extra random draws to reconstruct a selection trace; unavailable historical stages remain unavailable. `unscored` is grading disposition, never an investigation-resolution state (A05/A06/A08/A12).
...
### 6.4 Future Strategic Sanity experiment and promotion boundary

This is a future experiment specification, **not an authorized implementation task or production guard**. Current T2/T5/T6/T7/T8 provide the reusable evidence and reproduction seam; T10/T11 can supply independently evaluated reviewer evidence; T15 checks the seam, import boundary and unchanged action selection. Passing T15, reviewer validation or a review-grade calibration grants no guard authority.

A separately authorized offline/shadow experiment must first reproduce suspicious states and classify root cause, including legitimate unusual negative controls. Compare A: original selected action and immutable original trace; B: scoped shared assessment without intervention; C: hypothetical proposed intervention (allow/abstain/remap/regenerate/suppress) with explicit reason, legal semantic action and independently evaluated value change. All arms use matched states/deals, separate deterministic experiment streams, grouped uncertainty, holdout isolation and versioned intervention rules. Record abstentions, coverage and missing reference support rather than pretending all hypothetical changes have known benefit.

Candidates independently proven illegal or malformed may be proposed for removal; unsupported or aesthetically strange is not mechanically invalid. Prefer fixing the demonstrated generator/model/algebra fault. If a bounded re-selection experiment is justified, specify at most one additional selection with a separate deterministic stream and an explicit abstention outcome; never reroll indefinitely until the action looks normal. Shadow work cannot affect live candidates, commands or RNG consumption.

Report false positives on supported unusual lines (including 5x/10x bets), action/EV changes against independent references, effects by event family/objective/support status, root-cause recurrence and whether exclusions or reselection mask defects. Freeze empirically justified acceptance criteria before holdout. Require real evidence and explicit human methodology/promotion approval, then a separately authorized adoption change with scope, rollback and continued monitoring. Reviewer qualification and guard promotion are separate records; neither is manufactured by an agent. No intervention thresholds, live veto flags or new CI behavioral quotas are introduced here. A15 verifies this boundary; implementation of the future experiment itself remains outside T1–T15.
...
These identifiers supplement, not replace, the dimension-by-dimension `ConditioningEvidence` in section 4.7. Test an observed-size sweep of 33%, 75%, 150%, 500% and 1,300% pot, preserving other context where legal. Each must prove which inputs the model actually consumed and identify its domain/extrapolation. Merely changing a hash while applying an unconditional range fails. Equal conditional results are allowed when an explicit model justifies equality; do not force arbitrary range changes to pass. Unsupported extreme sizing remains a limitation even with a narrow numerical interval or exact enumeration. Finite synthetic models demonstrate plumbing and conditional conclusions, not empirical calibration of real opponents. Production range formulas remain unchanged; missing capability is reported, not silently repaired in this task.
...
Before menu evaluation, collect the opponent's relevant observed prefix actions and assess their support in Hero's historical information scope, alongside Hero's candidates. Obtain each model's conditioning/domain evidence through the injected provider. Unsupported observed-action inference limits strategic confidence even when every candidate transition is legal. Evaluate available, predeclared plausible sensitivity models; never select only favorable variants. The shared assessment itself returns no grade. A recommendation may remain robust across supported models despite unusual sizing; unsupported domain coverage remains explicit and cannot be erased by agreement among arbitrary model guesses.
...
Required plateau tests use injected, analytically specified reference surfaces: 11,063 = 14.82 BB, 11,000 = 14.81, 11,500 = 14.80 and 12,000 = 14.77. With explicitly synthetic epsilon 0.10 BB and exact supported values, all four receive `excellent` against that finite menu. A different supported surface makes 12,000 = 11.40 and returns supported loss; noisy paired bounds crossing epsilon yield unresolved rather than excellent/loss; a discontinuity between supported endpoints forbids one continuous interval. These fixture values/tolerance are test data, not calibrated poker claims or production thresholds. Add at least one actual finite-world poker reference case exhibiting multiple equivalent choices so T8 is not only testing synthetic curves. No raw 10–15% size-distance rule can substitute for these EV tests.

Use a one-chip legal context for the literal 11,063 surface fixture. In a 25-chip context, 11,063 may describe an abstract optimum but must not be advertised as a physically feasible ordinary generated wager. This distinction never disqualifies the player's supported 11,000/11,500/12,000 choices.
...
For genuinely exhaustive/certified evidence the display may say “Preferred sizing region: ~11k–12.5k; You bet: 12k; Result: Near-optimal / equivalent.” For the four discrete fixture points, show the supported sampled sizes instead; do not infer 12.5k or the intervals between samples. Neither view says “Correct bet: 11,063.” A08 verifies both rendering payloads and their coverage basis.
...
### 7.3 Reconstructed extreme-jam semantic regression (A04/A07/A08)

Retain the report's approximate historical observations as provenance only: Hero Q♦T♦ on T♥A♦J♥, roughly 1.6k–1.7k before opponent pressure, roughly 22k+ terminal wager; displayed P=22,467, C=20,282, P+C=42,749, required equity 47.4%, modeled equity 62.7%, responders 0; old values Call +37.18±9.15 BB, All-in +30.06±9.87 BB, Fold 0±4.81 BB, labeled Blunder. These are neither reference values nor a recovered exact replay. The approximate pot/wager descriptions cannot all be imposed as exact state facts.

T4 builds a **synthetic reconstructed** reachable three-seat case with one-chip rack, blinds 50/75, no ante: button opens to 185, small-blind Hero raises to 1,000, big-blind opponent calls 1,000, button folds. Hero and opponent each started with 21,282 chips; button has sufficient chips for its legal actions. This makes a 2,185 pot and leaves both active players 20,282. On the specified flop Hero checks, opponent jams 20,282, Hero faces C=20,282 with exactly that stack remaining. P=22,467 and P+C=42,749, odds C/(P+C)=47.44438…%; no uncalled refund or side pot in this case. The reconstructed prior pot and jam deliberately differ from the approximate narrative; do not assert the historical screenshot used these blinds/stacks. Specify all cards/deck and engine action order in fixture provenance, with opponent worlds supplied as model assumptions, never imported from actual hidden truth.

Required assertions, owned by the stated tasks:

1. A04 proves the prefix legal and all commitments/pot/stacks independently reconcile. A07/A08 canonicalize call and explicit all-in into one transition and one value/sample stream; target equals current bet, so no distinct raise branch or extra action probability exists. Opponent is already all-in; after Hero calls betting closes, while turn/river chance remains.
2. A07 proves incremental fold value exactly zero with zero Monte Carlo width; call uses exact finite-world/runout enumeration where budget permits, otherwise declared chance-only sampling, with no future strategic rollout. Compare exact arithmetic/payoff bounds independently. Responders zero must not remove the opponent from showdown contestants.
3. A07/A08 exercise insufficient extreme-size conditioning, supported unusual conditioning, robust-call supported range variants, and sensitivity variants that reverse or fail to resolve the preference. Equity wording is always “against the modeled/assumed range.” No test assumes historical 62.7% or old BB values are true. Numerical convergence cannot increase domain confidence.
4. A08 permits a strong Call recommendation when it survives all relevant supported plausible models; a fold's supported loss/severity still requires robust regret and calibrated criteria. Sensitive or unsupported inference yields unresolved/OOD/unscored, never a confident Blunder or automatic excellence. The absence of real-world calibration remains visible even if the synthetic provider is exact.
5. A05/A06/A08 retain the opponent selected-action trace or explicit historical-unavailable reason, exclude unreliable Hero grades, and count every exclusion in action/category coverage. Removing a grade never resolves the bot investigation. A08 tests that changing hidden cards while keeping observer-visible/model inputs fixed leaves the public assessment and analysis unchanged.

T4 additionally builds independently supported unusual 5x- and 10x-pot negative controls with recorded stack/exposure, SPR, street/board, prior sequence, boundary origin and model domain. A07/A08 verify their support and potential near-optimality from reference evidence; magnitude alone must not make them invalid, bad, blocked, regenerated or unscored. Cases lacking empirical labels remain honestly pending; synthetic reference correctness is confined to its declared model.

### 7.4 Grade interpretation and action-category diagnostic audit (A06/A08)

Keep internal `grading.quality` as `excellent`/`supported_loss` or null; an eventual calibrated display mapping may distinguish model-best/excellent (supported tied set), good/near-optimal (negligible loss), inaccuracy (noticeable loss), mistake (material loss), and blunder (large robust loss). This conceptual ordering supplies no numeric thresholds. Each severity mapping requires approved objective/context/model scope and regret-bound criteria. Point ordering, chip-distance, denomination, unresolved sensitivity or OOD cannot generate severity. Strange opponent behavior cannot make Hero automatically correct. If relevant robust bounds cross a severity boundary, leave that severity unresolved. A08 tests these exclusions even when a legacy label would have been strong.

The historical approximately 44-decision display (folds 89%, checks 100%, calls 9%, bets 13%, raises 100%) is an audit motivation, not an expected distribution or proof of a bug. T8's offline runner sends representative generated/replayed decisions to the T6 generic reducer. For each canonical fold/check/call/bet/raise category report total encountered, grade-eligible count, supported count, OOD/unresolved/pending/invalid/unsupported-objective counts, exclusion reasons, regret distribution with bounds and supported-subset denominator, nullable grade distribution, model-support distribution, independent family/block counts and trace availability. Use exhaustive `decisionStatus` counts so encountered equals the sum of mutually exclusive statuses, including unsupported objectives; mirror these counts in `ReviewEvidenceEnvelopeV2`. Grade-eligible is a separately reconciled subset. Model-status marginals may overlap and must be labeled. Keep grade eligibility distinct from behavioral opportunity eligibility and never average objective units together.

Audit matched/reference cases for continuation assumptions, unsupported response branches, candidate coverage, pot/call arithmetic, exact-size matching, range conditioning, duplicate aliases, uncertainty and passive/aggressive production-utility leakage. Include deliberately corrupted fixture evidence for each mechanism and clean counterparts; detecting corruption must not require aesthetic rate quotas. Report root-cause evidence and unresolved limitations, not “calls must be at least 60% good.” A06 verifies hand-counted aggregate reconciliation including all-unscored categories; A08 verifies reference-backed cases and exclusions feed that report; A15 verifies the report is in the all-task output.
...
Implement section 4.7 contracts and `assessActionSupport` in new `src/lib/actionSupport.ts` with `src/lib/actionSupport.test.ts`; later evidence remains explicitly unavailable. A02 additionally proves same scoped payload yields identical consumer assessments, orthogonal legality/support/unusualness, reason/measurement completeness, unknown handling, deterministic no-RNG behavior and safe projection.
...
Include section 7.3 reconstructed jam and 5x/10x supported-unusual negative-control families; A04 verifies provenance, reachability, exact arithmetic and family isolation before T7 adds model evidence.
...
A05 additionally captures the section 4.7 stage trace and immutable scoped assessment links. Test successful joins, wrong-replay rejection, historical missing traces, unavailable later-producer fields, root-cause stage distinctions and unchanged seeded actions.
...
Own the generic `ActionAuditRow` contract and reducer in existing allocated T6 modules; no T8 import. A06 tests section 7.4 counts/distributions and all-excluded/null cases, support-scope separation, preserved investigation links and unresolved causes despite grade exclusion.
...
T7 populates section 4.7 evidence through the T2 seam and immutable revisions, without rewriting T5 records. A07 covers section 7.1 dimension evidence/33%–1,300% sweep (including justified equality and rejected hash-only conditioning), supported-unusual controls, and section 7.3 exact closure/zero-fold/conditional-equity references. Neither tighter MC intervals nor a critic verdict upgrades unsupported model scope.
...
T8 consumes `assessActionSupport` for relevant observed opponent actions and candidate actions in Hero scope, and sends audit rows to T6 only from `reviewPrototypeRunner.ts`. A08 includes all section 7.3 properties, hidden-truth substitution, robust-call and model-sensitive variants, section 7.4 severity exclusions and diagnostic corruption controls, and unchanged internal investigation identity after exclusion. No guard or live UI wiring.
...
A10 also proves shared support conclusions and source diagnostics cannot reach an initial blinded verdict; only observer-safe evidence enters separately locked diagnosis. T10 consumes T2 schemas, not T7 implementations.
...
A11 compares shared support hypotheses against independent reference/human judgments, reports false positives on legitimate unusual cases and abstentions by support domain, and keeps initial blinded judgments separate from post-evidence diagnosis. Agreement with the shared classifier is not independent gold; reviewer validation cannot promote that classifier or a guard. Missing human evidence remains pending.
...
A12 additionally links actual Normal probability/rank, best/deviation branch and unchanged stochastic selection evidence to section 4.7 investigation records without exposing private policy data to Hero review; missing earlier Normal evidence is supplemented immutably.
...
A15 additionally verifies both consumers use the same pure assessment for the same scoped payload, all new fields have implemented producers/consumers and version validation, the jam/plateau/conditioning/negative-control/audit tests execute, and unscored actions retain traceability. Validate the public DTO with hidden-truth substitution and traverse imports to exclude scripts/guard/reviewer authority from live selection. No guard task or production promotion is required or authorized; reports say future intervention validation/adoption pending.
...
Allowed production source edits are strictly: shared pure semantics/support assessment/DTOs; unchanged helper exports; additive telemetry; common traversal/adapter/selection preparation preserving defaults; and explicit nondefault full-field lifecycle options. T1/T3/T4/T6/T9/T10/T11/T13/T14 are measurement/tooling only; T2/T5/T7/T8/T12 may touch source under those restrictions; T15 changes tooling. No task activates persistent learning, new wager preference or critic control in live games.
...
| Invent separate behavioral and review plausibility heuristics | One deterministic assessment with observer/model scope | A02/A15 identical payload yields identical evidence |
| Mark any 5x/10x bet illegal, bad or automatically unscored | Orthogonal mechanical/support/unusualness evidence and negative controls | A04/A07/A08 supported unusual lines remain analyzable |
| Claim range conditioning by changing only a size hash | Per-dimension consumed inputs and domain evidence | A07 size sweep rejects unconditional model disguised as conditioned |
| Read tight sampling bounds as trustworthy opponent assumptions | Numerical and model confidence separate; supported sensitivity scope | A07/A08 exact but unsupported and model-sensitive cases remain unscored |
| Split terminal Call/All-in or give fold noisy EV | Reconstructed legal prefix, transition dedup, chance-only closure | A04/A07/A08 jam fixture has one call value, no raise and exact zero fold |
| Hide generator defect behind an OOD grade exclusion | Internal stage trace and independent investigation state | A05/A06/A08 excluded grade retains cause links and coverage |
| Leak opponent cards indirectly through a sanitized support label | Recompute in Hero information scope; safe public projection | A08/A10 hidden-truth substitution leaves public evidence unchanged |
| Balance suspicious action grades using quotas | Counts, regret/support distributions and corruption controls | A06/A08 audit reports evidence without target percentages |
| Promote a guard after reviewer or infrastructure tests pass | Separate future experiment/human promotion/adoption, no live hook | A15 import/default-action tests and pending promotion report |
...
|---|---|---|---|
| C1 | T1–T3 | Shared scoped support/reasons/projection/no-RNG tests; A01–A03; engine betting/pots; player-count semantics; measurement and semantic all-in audit tests; Wesley fixture | Canonical alias/denominator/geometry contracts pass and artifacts cannot overwrite. |
| C2 | T4–T6 | Reconstructed jam/5x/10x reachability, investigation joins and generic action-audit counts; A04–A06; session/runner/critic sampling tests; independent second seeded run; complete tiny field after hero bust | Reachability/split isolation, full-field vs hero, cluster math and raw trace reconciliation pass. |
| C3 | T7–T9 | Conditioning sweep, jam closure/alias/zero fold, sensitivity, observer-safe support and action-category audit; A07–A09; Rational/equity-service/hand-review compatibility tests; Wesley; flat/adverse/noisy/unsupported surfaces | Independent reference is real, default policy unchanged, review/denomination boundaries hold. |
| C4 | T10–T12 | Initial support blindness, independent hypothesis validation and Normal trace; A10–A12; Normal/league tests; import-exclusion tests; 8-base mock pilot with transformations | No leaks/network default; all reviewer authority false; profile probability extraction preserves commands. |
...
- [ ] Review v2 has real paired/exact evidence, uncertainty and OOD/pending outcomes, and preserves the 11,063/11,000/11,500/12,000 tests without numeric-distance grading.
...
- [ ] A02/A15 establish one pure scoped support definition; legality, unusualness, EV quality, grading eligibility and authority are distinct. Every section 4.7 field has a tested producer/consumer, including explicit unavailable states before later producers.
- [ ] A07 verifies per-dimension actual range conditioning across 33%, 75%, 150%, 500%, 1,300% sizes, justified equal results and unsupported extrapolation; hash changes or tight MC bounds cannot invent domain support.
- [ ] A04/A07/A08 execute the labeled reconstructed Q♦T♦ / T♥A♦J♥ fixture: reachable geometry, equivalent call/all-in, no distinct raise, exact zero fold, chance-only closure, conditional equity and robust/sensitive model variants.
- [ ] A08 reviews observed opponent actions in Hero's information scope without hidden-truth-derived labels; supported unusual 5x/10x controls are not automatically prohibited or excluded.
- [ ] A05/A06/A08/A12 preserve stage-level selected-bot investigation links through OOD exclusions, including explicit missing historical trace and Normal/tournament/RNG causes; unscored never means resolved.
- [ ] A06/A08 reconcile action-conditioned encounter/eligible/support/exclusion counts and regret/grade/model distributions; historical 44-decision percentages are motivation only, not truth or quotas.
- [ ] A08 rejects severity from size distance, point ordering, unresolved bounds or OOD; calibrated model-scoped regret remains primary and unsupported intervals cannot become a continuous sizing region.
- [ ] A10/A11 keep initial support judgments blinded, diagnosis separate and reviewer qualification independent from classifier or guard authority.
- [ ] A15 confirms the shared evidence seam works end to end with no guard module/live selection hook; future A/B/C intervention experiments and human promotion/adoption remain outside T1–T15.
...
| Two consumers disagree about the same action | Same normalized input, observer scope, model and evidence produce the same assessment. Different scoped assumptions may differ explicitly; no independent consumer heuristics. |
| New task/dependency needed for shared support | T2 owns pure contract/assessment, T5 trace, T6 report, T7 reference producer, T8 review. Add direct T6→T8 for the offline diagnostic reducer; exact T1–T15 order and six checkpoints remain. |
| Exact screenshot reconstruction | Historical figures are approximate observations; section 7.3 declares a reachable synthetic state preserving displayed P/C arithmetic. Actual historical replay/cause cannot be recovered without original evidence. No invented historical truth. |
| Extreme sizing vs supported confidence | Size triggers scrutiny; actual conditioning/domain and sensitivity evidence decide model support. Equal conditioned ranges require justification; no automatic large-bet prohibition. |
| Unscored review vs generator defect | Grade exclusion and investigation disposition are separate; retain internal telemetry joins while public views remain observer-safe. |
| Future Strategic Sanity adoption | No T16 needed to implement the current evidence seam. Section 6.4 specifies later offline/shadow validation and explicit human promotion/adoption, outside current authorization. T15 success does not approve it. |
| Human methodology decisions still pending | Real range-domain calibration, plausible sensitivity-family coverage, strategic rarity criteria, epsilon/severity policies and intervention acceptance/promotion need evidence and human approval. Synthetic fixtures supply none of those approvals. |
