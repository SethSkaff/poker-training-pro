...
|---|---|---|
| M0 — decision, budgets, bootstrap, fallback | **Partial** | Decision, dependency, lazy import, preview setting, named scene budgets, readiness/fallback state, and the existing 2.5D path exist. D3D-F05/F06 prove normal and forced-failure packaged fallback, visible-canvas/accessibility parity, native-minimize frame freeze, and three trusted, default-prevented browser `WEBGL_lose_context` recoveries with stable renderer-local resources. Missing: a blocklist/quality policy and low-spec support evidence; extension-triggered recovery is not a substitute for a hardware driver-failure matrix. |
| M1 — exact one-table vertical slice | **Partial** | A real room/table, seated camera, primitive cards/chips/bodies, six public seats, and real event integration exist. D3D-F06 proves normal and forced-failure packaged complete-hand paths. Missing: physical card identities; dealer puck; blind markers; an explicit current-turn treatment; distinct deal, check/call, bet/raise, and fold actions keyed by event; and coherent DOM/canvas composition. |
