...
|---|---|---|
| M0 — decision, budgets, bootstrap, fallback | **Partial** | Decision, dependency, lazy import, preview setting, named scene budgets, readiness/fallback state, and the existing 2.5D path exist. D3D-F06 now proves normal and forced-failure packaged fallback, visible-canvas/accessibility parity, native-minimize frame freeze, and classified synthetic loss/recovery. Missing: a blocklist/quality policy and repeated real driver-loss/low-spec evidence. |
